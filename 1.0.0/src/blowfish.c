...
{
	// Maximum length of a blowfish key being 448 bits, or 56 bytes
	if(keyLen > 56 || strlen((const char *)key) > 56) return;
  	...
  	for (i = 0; i < N + 2; ++i)
	{
    		...
    		for (k = 0; k < 4; ++k)
		{
			/*
				...
				As suggested by Mike Morgan, see:
				https://www.schneier.com/wp-content/uploads/2015/12/blowfish-bug-2.txt

				Shouldn't necessarily be a problem, since Paul Kocher already changed
				the 'key' argument to necessitate an unsigned char...
			*/
      			data = (data << 8) | key[j];
      			j = j + 1;
...
/*
	Below is new code written by anson.
*/
